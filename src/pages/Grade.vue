<template>
    <div>
      ...
      <div class="search">
        ...
        <label>学号:<input type="text" placeholder="请输入学号" v-model="keyWord3"/></label>
        <label>姓名:<input type="text" placeholder="请输入姓名" v-model="keyWord4"/></label>
      </div>
      <el-table :data="gradeData" :height="height" :default-sort = "{order: 'descending'}">
        ...
        <el-table-column label="课程号" width="160">
          <template slot-scope="scope">
            <span>{{ scope.row.cno }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="成绩" prop="grade" sortable width="120">
          <template slot-scope="scope">
            <span>{{ scope.row.grade }}</span>
          </template>
        </el-table-column>
    ...
    computed:{
              gradeData(){
                 return this.tableData.filter((p)=>{
                      return p.cno.indexOf(this.keyWord1) !== -1 && p.cname.indexOf(this.keyWord2) !== -1 && p.sno.indexOf(this.keyWord3) !== -1 && p.sname.indexOf(this.keyWord4) !== -1 
                  })
              }
          },
    methods: {
      ...
      getGradeTable() {
        this.$axios({
          url: "http://127.0.0.1:5000/home/grade",
          method: "get",
        }).then((response) => {
      ...
      deleteGrade(cno,sno) {
        this.$axios({
          url: "http://127.0.0.1:5000/home/grade/delete",
          method: "get",
          params: { 
            cno: cno, 
            sno: sno
      ...
      updateEdit(params) {
        this.$axios({
          url: "http://127.0.0.1:5000/home/grade/update",
          method: "get",
          params: {
            Cno:params.cno,
            Sno:params.sno,
            Grade:params.grade,
            Gremarks:params.remarks
          },
        }).then((response) => {
      ...
      handleDelete(index, row) {
        if (confirm("是否确认删除？")) {
          this.deleteGrade(row.cno,row.sno);
        }
      },
    },
